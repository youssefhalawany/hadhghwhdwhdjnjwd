...
import { Store, Languages, Wifi, Clock, Sparkles } from "lucide-react";
...
export function MobileHeader() {
  ...
  return (
    <header
      className="sticky top-0 z-40 w-full bg-slate-950/90 border-b border-slate-800/80 backdrop-blur-2xl px-3 py-2.5 md:hidden no-print"
      dir={isAr ? "rtl" : "ltr"}
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-900 border border-slate-800 text-[10px] font-bold">
            <span
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-[11px] font-extrabold transition-all active:scale-95"
          >
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
        <Store className="w-3.5 h-3.5 text-slate-500 shrink-0 ml-1" />
        {branches.map((b) => {
